/**
 * Browser verification of the mortar-bed block-tile roof node.
 *
 * Checks:
 *   1. NodeDetail loads: title, canvas, 5-button whitelist bar, 1 enabled slider.
 *   2. Explosion: 播放爆炸 advances slider (>0), 收起爆炸 returns to 0.
 *   3. R reset returns slider to 0.
 *   4. Picking: click on the mesh → right-side knowledge panel shows layer info.
 *   5. Library page lists the new node.
 *   6. No console errors, no failed network requests.
 *
 * Usage: node scripts/verify-mortar-bed-node.mjs <baseUrl>
 */
import { chromium } from "playwright";

const baseUrl = process.argv[2] ?? "http://localhost:5173";
const VIEWPORT = { width: 1440, height: 900 };
const NODE_ID = "mortar-bed-tile-roof-01";
const SHOT = "audit-output/AUDIT_EVIDENCE/node-mortar-bed-tile-roof.png";
...
ok(/砂浆卧瓦/.test(heading), `header breadcrumb shows node title ("${heading.trim()}")`);
...
// Click near model center (roof is auto-fitted to viewport).
...
const hasWagon = /卧瓦|块瓦|挂瓦/.test(panelText);
ok(hasWagon, `right-side knowledge panel shows layer info (卧瓦/块瓦/挂瓦)`);
if (!hasWagon) {
...
ok(/砂浆卧瓦块瓦屋面构造/.test(libText), "library page lists the new node");
